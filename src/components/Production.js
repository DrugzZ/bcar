...
class Production extends Component {
  ...
  render() {
    ...
    if (this.state.isLoading) return <p className="text-dark">Loading...</p>;

    return (
      <div className="wrap text-dark">
        <ProdSubMenu match={match} data={data} />
        <Route
          path={`${match.path}/:subgroup`}
          render={props => <ProdCategory {...props} data={this.state.data} />}
        />
        <Route
          exact
          path={`${match.path}`}
          render={props => <Categories {...props} data={this.state.data} />}
        />
      </div>
    );
  }
