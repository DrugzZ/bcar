...
class ProdCategory extends Component {
  ...
  componentDidMount() {
    ...
    let machRef = db
      .collection("machines")
      .where("subgroup", "==", this.props.match.params.subgroup);
      ...
      .then(
        function(querySnapshot) {
          ...
          localStorage.setItem(
            this.props.match.params.subgroup,
            JSON.stringify(docs)
          );
  ...
  render() {
    ...
    const subgroup = this.props.data.filter(
      subgroup => subgroup.title === this.props.match.params.subgroup
    );
    ...
    return (
      <div className="container pb-5">
        ...
        <nav className="d-flex justify-content-md-center text-uppercase subgroup--submenu">
          {data.map((mach, index) => {
            ...
            return (
              <NavLink
                key={index}
                activeClassName="subgroup--active d-inline-block"
                className="py-2 px-3"
                exact
                replace
                to={this.props.match.url + first}
              >
                {mach.model}
              </NavLink>
            );
        ...
        <Route
          ...
          component={props => <ProdSpecs {...props} data={data} />}
        />
        <Route
