...
class ProdSpecs extends Component {
	...
	handleChangeEngine(e){
    	this.setState({ engine: e.target.value, engineSpecs: this.state.model.engines[e.target.value] });
  	}
  	...
  	static getDerivedStateFromProps(nextProps){
		...
		if(!param) {
  		...
  		return {
  			...
  			height: model.heights[0].name + model.heights[0].options[0].liftHeight.value
  		}
  	}
	...
	render() {
		...
		return <div className="bg-white px-5 py-5 b-sh">
					<h3 className="mb-4">{`${capFirst(model.type)} ${model.model.toUpperCase()}`}</h3>

					<div className="row">
					<div className="col">
					{engineOptions.length > 1 ? <FormControl component="fieldset">
						...
						<RadioGroup
							...
							>
								{engineOptions.map((option, index) => 
									<FormControlLabel key={index} value={option} control={<Radio />} label={this.state.model.engines[option][0].value} />
								)}
						...
						<RadioGroup
							...
							>
								{model.heights.map((liftType, index) => 
									<FormControlLabel key={liftType.name} value={liftType.name} control={<Radio />} label={capFirst(liftType.name)} />
								)}
					...
					<div className="col">
				        ...
				        <FormControl style={formStyle}>
				          <InputLabel style={{fontSize:'1rem', transform: 'scale(1)', marginBottom: '.5rem'}} shrink htmlFor="pickHeight">Высота подъема:</InputLabel>
				          <Select
					...
					<div className="row">
						{model.specs.map((spec) => {
							...
							let devider = spec.orderNum % 3 === 0 ? <div className="w-100"></div> : '';

							if(spec.orderNum === 6) {
								return <React.Fragment key={spec.orderNum}>
